// Package clrsbinary implements the exact-program binary-search vertical
...
// no-hint prompt/reference grammar and lower-bound binary-search operation.
func PinnedSourceEvidence() SourceEvidence {
	return SourceEvidence{
		...
		Algorithm: SourceFile{
			Path:    "clrs/_src/algorithms/searching.py",
			GitBlob: "bc59a9eae7d86881b918d08a16279493ac6dc7a6",
			SHA256:  "4a410b290d94efacb112235757185dcafe97d27a59b4faec29e6196a5f0af117",
		},
		Sampler: SourceFile{
...
func (evidence SourceEvidence) ValidateSource(source clrsfixture.SourceRecord) error {
	if evidence != PinnedSourceEvidence() {
		return fmt.Errorf("binary-search supporting source evidence differs from the pinned record")
	}
	identity, err := source.Identity()
	if err != nil {
		return fmt.Errorf("validate binary-search source record: %w", err)
	}
	if identity.String() != evidence.SourceID {
		return fmt.Errorf("binary-search source identity = %s, want %s", identity, evidence.SourceID)
	}
	return nil
}
